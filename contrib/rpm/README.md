...
The RPM spec file provided here is for Titancoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://titanprojects.co/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Titancoin git repository](https://github.com/titancoinproject/Titan) but are not part of
...
Currently only `Source10` is used, for the example `titancoin.conf` file.
...
in the titancoin git repository. At present only an SVG version of the Titancoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the titancoin
...
needed when 0.12.1 is released, a proper fix is already in the Titancoin git
master branch.
...
idea to build Titancoin Core against a static version of that library compiled
...
The `titancoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `titancoin-server` package. At this point, most RPM based
...
The `titancoin-server` package should have SELinux support. How to properly do
...
Titancoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/titancoin/). He has been
packaging Titancoin for Fedora at least since 2012.
...
4. I renamed the `titancoin` package that contains the Qt GUI to `titancoin-core` as
that appears to be how the general population refers to it, in contrast to
`titancoin-xt` or `titancoin-classic`. I wanted to make sure the general population
