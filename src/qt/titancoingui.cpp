...
const std::string TitancoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString TitancoinGUI::DEFAULT_WALLET = "~Default";

TitancoinGUI::TitancoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void TitancoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Titancoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and titancoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to TTN addresses"));
    ...
    manageAssetAction->setStatusTip(tr("Assets will active on block number 86400"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Titancoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Titancoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a titancoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Titancoin command-line options").arg(tr(PACKAGE_NAME)));
...
void TitancoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void TitancoinGUI::createToolBars()
{
    if(walletFrame)
    {
        /** TTN START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/titancointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* titancoinLabelLayout = new QVBoxLayout(toolbarWidget);
        titancoinLabelLayout->addWidget(label);
        titancoinLabelLayout->addWidget(toolbar);
        titancoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        titancoinLabelLayout->addStretch(1);
...
void TitancoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool TitancoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool TitancoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void TitancoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void TitancoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void TitancoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void TitancoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void TitancoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void TitancoinGUI::optionsClicked()
{
...
void TitancoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void TitancoinGUI::showDebugWindow()
{
...
void TitancoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void TitancoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void TitancoinGUI::openClicked()
{
...
void TitancoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void TitancoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void TitancoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void TitancoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void TitancoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void TitancoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** TTN START */
void TitancoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void TitancoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void TitancoinGUI::gotoManageAssetsPage()
{
...
void TitancoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Titancoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void TitancoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void TitancoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void TitancoinGUI::updateHeadersSyncProgressLabel()
{
...
void TitancoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void TitancoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Titancoin"); // default title
...
void TitancoinGUI::changeEvent(QEvent *e)
{
...
void TitancoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void TitancoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void TitancoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "TTN")
        msg += tr("Amount: %1\n").arg(TitancoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(TitancoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void TitancoinGUI::checkAssets()
{
    ...
    if(AreAssetsDeployed()) {
        ...
        transferAssetAction->setToolTip(tr("Assets will active on block number 86400"));
        createAssetAction->setDisabled(true);
        createAssetAction->setToolTip(tr("Assets will active on block number 86400"));
        manageAssetAction->setDisabled(true);        
        }
}
#endif // ENABLE_WALLET

void TitancoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void TitancoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool TitancoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool TitancoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void TitancoinGUI::setHDStatus(int hdEnabled)
{
...
void TitancoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void TitancoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void TitancoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void TitancoinGUI::detectShutdown()
{
...
void TitancoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void TitancoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void TitancoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(TitancoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void TitancoinGUI::subscribeToCoreSignals()
{
...
void TitancoinGUI::unsubscribeFromCoreSignals()
{
...
void TitancoinGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<TitancoinUnits::Unit> units = TitancoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const TitancoinUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(TitancoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (TitancoinUnits::Unit u : TitancoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(TitancoinUnits::name(u)), this);
...
{
    setText(TitancoinUnits::name(newUnits));
}
...
void TitancoinGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=TTNBTC"));
