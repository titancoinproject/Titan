 ...
 * between titancoind, and titancoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("titancoin-shutoff");
    ...
    FlushWallets();
#endif
    GenerateTitancoins(false, 0, Params());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), TTN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/titancoinproject/Titan>";
    const std::string URL_WEBSITE = "<https://titanprojects.co>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("titancoin-loadblk");
...
/** Sanity checks
 *  Ensure that Titancoin is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Titancoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", TTN_CONF_FILENAME)).string());
    ...
    GenerateTitancoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
